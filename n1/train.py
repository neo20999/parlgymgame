...
#在原有的基础上，删除了需要atari rom的atari.py，对utils.py进行了处理，能够直接使用gym.make游戏
...
MEMORY_SIZE = 5e4
MEMORY_WARMUP_SIZE = MEMORY_SIZE // 10
IMAGE_SIZE = (84, 84)
CONTEXT_LEN = 4
FRAME_SKIP = 4
UPDATE_FREQ = 4
GAMMA = 0.99
LEARNING_RATE = 1e-3#原始3#-4、3#-3
...
def main():
    env = get_player(args.game_name,image_size=IMAGE_SIZE, train=True)
    #print(env)
    # test_env = get_player(args.game_name,image_size=IMAGE_SIZE,context_len=CONTEXT_LEN)
    ...
    with tqdm(
            total=MEMORY_WARMUP_SIZE, desc='[Replay Memory 启动]') as pbar:
